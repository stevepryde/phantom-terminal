...
| Header actions | Put at most one infrequent management action at the far right of a plugin header instead of adding another content row. Use a familiar icon, a full-height hit target, and a tooltip; clicking it must not collapse the section. Tasks uses this pattern to edit the exact discovered `.phantom.yml`. |
