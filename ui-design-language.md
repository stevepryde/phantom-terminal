...
| Context sidebar plugins | Runnable plugins show actions only in their steady state. Never show item counts, Details/Hide controls, source paths, commands, working directories, or explanatory metadata there. A single icon-only management action may share the plugin header row. Non-executable manual references are the only exception: they must be muted, explicitly labeled as manual, and have no hover, focus, pointer, or click affordance. |
...
| Chrome hover | Tabs, close buttons, New Tab, Settings, and Linux window controls use brief background-opacity fades on hover. Hovering a close button also keeps its parent tab hovered. Close and Settings icons fade toward the current UI theme accent; the Linux window close control uses the danger hover fill while minimize and maximize use the neutral chrome fill. Tab, close, New Tab, Settings, and Linux window-control hover targets use the pointer cursor; none scale on hover. |
...
| Contextual actions | Directory-aware actions use an edge-attached egui sidebar on the right of the main content area only; the titlebar and horizontal tab chrome always continue to the window's right edge. The open sidebar remains visible even when the current directory has no actions, reserves terminal glyph, cursor, selection, mouse, and scrollbar space, and leaves the full-window backdrop visible through its translucent surface. Sidebar opacity is capped below opaque even when other panels are configured to 100%, so the backdrop cannot disappear accidentally. Its default width remains compact, but the user may resize it freely from `100px` up to `50%` of the live viewport. Viewport clamping is presentation-only and must never overwrite the persisted preferred width during maximize, restore, or live window resizing. The left divider has a compact drag corridor entirely inside the sidebar, with resize cursor and hover/active emphasis; it must never extend into the terminal or overlap the terminal scrollbar hit corridor. Open/closed state and width persist. Closing it removes the reserved space and leaves a floating icon at the content area's top-right that reopens it. Sections are dense accordions separated by one thin horizontal divider. Each plugin has a validated numeric order independent of discovery timing; `Directories` is always first, followed by stable slots for contextual `Tasks` and `Deploy` sections even when either is absent. Each header is a fixed `28px` full-width row with an `11px` plugin label and a small leading chevron: right when collapsed and down when expanded. Built-in labels are `Tasks`, `Deploy`, and `Directories`; do not substitute project names or paths for these headings. A plugin may add at most one icon-only management action at the header's right edge; its hit target fills the header height, it must not toggle the accordion, and it requires an accessible tooltip. The selected recent/frequent directory set is displayed in case-insensitive alphanumeric path order, never recency order. Directory rows are fixed `24px`, retain 12px primary text, no inter-row gaps, a full-row hover fill, and a full-row click that always opens a new tab rooted at the revalidated path. Paths beneath the user's home directory display with a leading `~`; long paths keep their ending visible by eliding from the start. Default content is action-first with no redundant panel heading or help text, except concise labels that distinguish safe new-tab or manual-reference behavior from PTY command injection. Deploy always shows its unlabeled operation dropdown and run action after bounded local parsing; merely discovering `deploy.yml` never grants a separate trust state. Do not repeat the project/directory name, add a Details row, or add an `Operation` field label. Nested submenu breadcrumbs appear as muted, non-selectable group headings and must never masquerade as operations. Execution still requires the explicit run action and a fresh source/operation verification. Task trust review is compact: one wrapped line per task showing the exact command (env prefix, quoted program and arguments) at no smaller than `11px`, with the working directory shown only when it differs from the project root; do not repeat the manifest path or wrap tasks in cards. Untrusted manifests expose Trust only while those exact command lines are presented for review. An idle sidebar must not capture terminal typing. During native live window resizing, keep the translucent surface but temporarily suppress the full-surface blur pass; restore blur once at the settled size. |
| Frequent commands | The sidebar plugin is scoped to the active tab and shows at most three muted manual-reference rows ranked by in-memory submission count. New and restored tabs start empty; commands and counts are never persisted. The section must visibly say that commands require manual typing, and its rows must expose no hover fill, focus, pointer cursor, click action, or synthesized PTY input. |
...
| Scrollback find | Render search as a compact single-row egui surface at the terminal pane's top-right. Keep it left of an open contextual sidebar and reserve the closed sidebar button's top-right footprint when the sidebar is hidden. The row contains only its text input, four stateful option icons, a compact non-interactive `N of M`/`0`/`10000+` result label, and Previous/Next icon buttons; it overlays rather than resizing the terminal grid. Use `4px` rounding, a visible focus ring, non-color toggle state, tooltips and accessible names, and at least `28px` icon targets. |
...
| Pattern | Rule |
| --- | --- |
| Default content | Show executable actions only. Do not repeat counts, project names, manifest or config paths, commands, working directories, descriptions, or help text already implied by the section header and action labels. The Frequent Commands manual-reference section is the sole non-executable exception and must be visibly labeled and non-interactive. |
...
| Progressive disclosure | Never add item counts or Details/Hide toggles. Task security details appear directly only while a `.phantom.yml` source is untrusted; error details appear only with an actionable recovery path. |
...
| Task trust review | Untrusted `.phantom.yml` task sources are the exception to action-only content: show every task as one exact single-line command (environment prefix, quoted program and arguments) before enabling Trust, plus the working directory only when it is not the project root — a cwd outside the root keeps its full path. Do not show the manifest path (the header's edit action opens the exact discovered file) or wrap tasks in framed cards. Once trusted, return to action-only content. Deploy configs have no trust review. |
...
| Future plugin test | Before shipping, verify that every visible line either performs an action, is essential to a pending security decision, explains an error the user can act on, or is an explicitly labeled non-interactive manual reference. Remove everything else. |
