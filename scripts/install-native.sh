...
# The release profile leaves symbols in place so Cargo never invokes the
# rustup rust-objcopy helper (broken rpath for libLLVM.dylib on macOS).
# Strip the shipped binary here so the install artifact is deterministic.
if command -v strip >/dev/null 2>&1; then
  ...
  warn "native strip tool unavailable; the install binary may retain symbols."
fi
